...
class SessionState:
    ...
    restored_hits: int = 0                                       # utterances the router restored to
                                                                 # a template — a CLAIM we understood,
                                                                 # counted apart from proof that we did
    ...
    def paraphrased(self) -> bool:
        ...
        ⚠️ Reads `template_hits` only, never `restored_hits`. A template match is *proof* we parsed
        the customer; a model restoration is a *claim* that we did. The distinction is load-bearing
        because this flag selects the patience branch of the depth policy: `stall_decay` 0.8 when we
        understand them, 0.2 when we do not. A wrong restoration counted here would buy the patient
        branch while the agent is actually lost — slow AND wrong, the worst square available.
        Whether a restoration should earn that branch is a measurement, not an assumption; until it
        is made, restorations move ranking and never patience.
        """
