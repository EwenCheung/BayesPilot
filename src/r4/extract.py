...
* `src.common.attributes.normalise()` and the simulator's own `classify_constraint()` both key off a
...
class AlignedExtractor:
    """Wraps an `LLMClient` and exposes the `.extract()` shape `src.common.parse` expects.
