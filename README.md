...
COPILOT_OFFLINE=1 PYTHONHASHSEED=0 python3 scripts/evaluation/evaluate.py --all   # the table above
```
...
|---|---|
| `python3 scripts/evaluation/evaluate.py` | the submission score on `public_set`, defaults only |
| `python3 scripts/evaluation/evaluate.py --all --ci --output runs/final_r5.json` | the four-dataset table above |
| `python3 scripts/evaluation/evaluate.py --dataset data/dev.jsonl --ci --scenarios` | one dataset, with CI and per-scenario breakdown |
