...
PROBE = (
    "import sys; sys.path.insert(0, {root!r}); sys.path.insert(0, {root!r} + '/techjam-conversational-search-main');"
    "from src.r3.agent import Agent;"
    "from evaluator.local_evaluator import catalog_index, evaluate, load_jsonl;"
    "c_ids, cats, prods = catalog_index({root!r} + '/assets/catalog.jsonl');"
    "rows = load_jsonl({root!r} + '/techjam-conversational-search-main/data/resplit_60_20_20/train.jsonl')[:40];"
    "res = evaluate(Agent({root!r} + '/assets/catalog.jsonl'), rows, c_ids, cats, prods);"
    "print('%.10f' % res['recommended_technical_score'])"
)
...
class TestDeterminism(unittest.TestCase):
    ...
    def test_r3_scores_identically_under_different_hash_seeds(self) -> None:
        """Different PYTHONHASHSEED must not change the score by even one digit."""
        a = self._score_in_fresh_process("1")
        b = self._score_in_fresh_process("2")
        self.assertEqual(a, b, f"R3 agent drifted between hash seeds: {a} vs {b}")
