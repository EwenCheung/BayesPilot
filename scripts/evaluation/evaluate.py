...
def main() -> None:
    ...
    for label, samples, wrap in jobs:
        ...
        for level in levels:
            ...
            # ⚠️ Read the CLIENT's counters, not the extractor's: the router path goes through
            # `interpret_operations`, which never touches `AlignedExtractor.calls`, so that counter
            # reads 0 on a tier that is working. A dead endpoint must not look like a quiet one
            # either, which is what `failures` is for (IMPORTANT.md §13.1.4).
            client = getattr(getattr(agent, "llm", None), "client", None)
            calls = getattr(client, "calls", 0) or 0
            failures = getattr(client, "failures", 0) or 0
